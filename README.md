...
| `/reply` + one email title per line | All-or-nothing reply-all: every title must be found, then ONE card shows each thread's own To/Cc (reply-all to its latest message) with an input box — fill in one content, press **Send**, and each thread gets the reply with its own recipients. Sent from `om@` via SMTP with proper threading (In-Reply-To/References), and each thread's **latest message quoted below the reply** in Lark Mail's own style (`From:/Date:/Subject:/To:/Cc:` block + body) so recipients get the collapsible "Show email thread" section, exactly like clicking Reply in Lark Mail (`TOBOT_REPLY_QUOTE_CHARS=0` disables). Known-bouncing addresses are auto-removed with a ⚠️ "invalid recipient — removed" note on the card (list configurable via `TOBOT_REPLY_BLOCKED_RECIPIENTS`) |
