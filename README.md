...
| `/reply` + one email title per line | All-or-nothing reply-all: every title must be found, then ONE card shows each thread's own To/Cc (reply-all to its latest message) with an input box — fill in one content, press **Send**, and each thread gets the reply with its own recipients. Sent from `om@` via SMTP with proper threading (In-Reply-To/References), and each thread's **latest message quoted below the reply** using Lark's own composer markup (`history-quote-wrapper` + `adit-html-block--collapsed`, ported from osedutybot's `/egsreply`), sent as a **single `text/html` part** — so Lark Mail renders the collapsible **"Show email thread"** with the previous mail's original formatting, exactly like clicking Reply All by hand. The embedded history is sanitized (active content and cid-images dropped, `<div>`s balanced); `TOBOT_REPLY_QUOTE_CHARS=0` disables quoting. Known-bouncing addresses are auto-removed with a ⚠️ "invalid recipient — removed" note on the card (list configurable via `TOBOT_REPLY_BLOCKED_RECIPIENTS`) |
...
| `/whoami` (`/myid`) | Your Lark **open_id** + the chat_id — needed for `DEPLOY_ALLOWED_OPEN_IDS`. Tag the bot and say "who am i" for the same thing (every command also logs the sender's open_id server-side) |
| `/deploy` | `git pull origin main` + restart the systemd service (`TOBOT_SERVICE`, default `tobot`). Natural text works too: "git pull and restart service". Restrict who may deploy with `DEPLOY_ALLOWED_OPEN_IDS` (comma-separated open_ids; empty = anyone who can address the bot) |
