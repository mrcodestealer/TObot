...
| `/reply` + one email title per line | All-or-nothing reply-all: every title must be found, then ONE card shows each thread's own To/Cc (reply-all to its latest message) with an input box — fill in one content, press **Send**, and each thread gets the reply with its own recipients. Sent from `om@` via SMTP with proper threading (In-Reply-To/References). Known-bouncing addresses are auto-removed with a ⚠️ "invalid recipient — removed" note on the card (list configurable via `TOBOT_REPLY_BLOCKED_RECIPIENTS`) |
